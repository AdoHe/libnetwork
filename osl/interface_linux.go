...
func (n *networkNamespace) BindVirtualIP(vip string) error {
	...
	return nsInvoke(path, func(nsFD int) error {
		...
		if err != nil {
			return fmt.Errorf("vip %s must be CIDR notation", vip)
		}
		addr := &netlink.Addr{
			IPNet: &net.IPNet{
				IP:   ip,
		...
		if err != nil {
			return fmt.Errorf("failed to bind virtual ip to loopback device: %v", err)
		}
		...
		if err := netlink.LinkSetUp(lo); err != nil {
			return fmt.Errorf("failed to up loopback device: %v", err)
		}
...
func setupArpAnnounce() error {
	if _, err := exec.Command("echo", "1", ">>", arpAnnounceFile).Output(); err != nil {
		return fmt.Errorf("failed to setup arp_ignore: %v", err)
	}
	return nil
}
...
const (
	...
	base            = "[ ]*%s:([ ]+[0-9]+){16}"
	arpIgnoreFile   = "/proc/sys/net/ipv4/conf/lo/arp_ignore"
	arpAnnounceFile = "/proc/sys/net/ipv4/conf/lo/arp_announce"
)
